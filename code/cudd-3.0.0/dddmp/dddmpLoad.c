  ...
  )
{
  ...
  // DFA 11-2-21. Valgrind complains buf may be uninitialized
  buf[0] = 0;
  // END DFA
  if (fp == NULL) {
    fp = fopen (file, "r");
  ...
  Hdr->nVars = 0;
  Hdr->nsuppvars = 0;
  // DFA 28-09-2019 Use ALL variable names
  Hdr->varnames = NULL;
  // END DFA
...
// DFA 29-01-2019 Also read all variable names
      if (matchkeywd(buf, ".varnames")) {
        Hdr->varnames = DddmpStrArrayRead (fp, Hdr->nVars);
        Dddmp_CheckAndGotoLabel (Hdr->varnames==NULL,
          "Error allocating memory.", failure);
