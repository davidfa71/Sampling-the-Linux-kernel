...
void synExp::flattenAnd(synExp* exp, std::set<synExp*>& ss) {
    // Destroys exp
    if (exp->isAnd()) {
...
void synExp::flattenOr(synExp* exp, std::set<synExp*>& ss) {
    if (exp->isOr()) {
...
void synExp::print(std::ostream& os) const {
    std::ostringstream ost;
    ost << "Error accessing print in base class synExp: " << this << std::endl;
    throw std::logic_error(ost.str());
    return;
}
...
synMacro::synMacro(std::string s) : synExp(), macro(s)     {};
